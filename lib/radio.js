...
/* harmony default export */ __webpack_exports__["a"] = ({
  methods: {
    ...
    dispatch(componentName, eventName, params) {
      ...
      let name = parent.$options.componentName; // 查找组件名为componentName的父组件
      ...
      console.log('父亲节点', parent, name, [eventName].concat('1')); // apply 修改this.$emit中的this指向
...
/* harmony default export */ var radiovue_type_script_lang_js_ = ({
  ...
  computed: {
    ...
    model: {
      ...
      set(val) {
        console.log('model值发生了变化', val); // 通知父组件更新v-model对应的value值
        // this.$emit('input', val)
  ...
  methods: {
    handleChange() {
      ...
      this.$nextTick(() => {
        console.log('handleChange', this.model); // 这句话有什么意义, 通知父组件选中状态变化后的回调

        this.$emit('change', this.model); // 如果是父组件是radio-group组件, 则调用父组件的handleChange, 以更新model值
